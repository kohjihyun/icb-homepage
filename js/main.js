(function () {
  ...
  /* ── section2-card: 카드 1회 등장 후 고정 → 도형 4개 순차 → 2초 후 도형만 반복 ── */
  function setupSection2Card() {
    ...
    /* 도형 순차 재생 단계: [클래스명, 단계 길이(ms)] */
    const SHAPE_STEPS = [
      { cls: 'shape-011-on', dur: 900  },  // 011 원 확장
      { cls: 'shape-012-on', dur: 900  },  // 012 두 사각형 (0s + 0.18s 시차 + 0.7s)
      { cls: 'shape-013-on', dur: 800  },  // 013 사각형 라이즈 (이후 플로팅은 백그라운드 진행)
      { cls: 'shape-014-on', dur: 1150 },  // 014 spin 1.1s
    ];
    const SHAPE_CLASSES = SHAPE_STEPS.map(s => s.cls);
    const SHAPE_TOTAL   = SHAPE_STEPS.reduce((a, b) => a + b.dur, 0);
    const GAP_MS        = 2000;   /* 도형 모션 종료 후 2초 대기 */
    const RESET_MS      = 700;    /* 리셋 트랜지션이 정착할 시간 */

    if (prefersReducedMotion) {
      cards.forEach(c => c.classList.add('is-in'));
      SHAPE_CLASSES.forEach(cls => wrap.classList.add(cls));
      return;
    }

    const lastCard       = cards[cards.length - 1];
    const lastDelaySec   = parseFloat(getComputedStyle(lastCard).getPropertyValue('--s2c-delay')) || 0;
    const CARD_REVEAL_MS = lastDelaySec * 1000 + 900;

    let timers = [];
    function clearAll() { timers.forEach(clearTimeout); timers = []; }

    function playShapesLoop() {
      clearAll();
      let acc = 0;
      SHAPE_STEPS.forEach(({ cls, dur }) => {
        timers.push(setTimeout(() => wrap.classList.add(cls), acc));
        acc += dur;
      });
      /* 시퀀스 완료 + 2초 대기 → 도형만 리셋 → 700ms 후 다시 재생 */
      timers.push(setTimeout(() => {
        SHAPE_CLASSES.forEach(cls => wrap.classList.remove(cls));
        timers.push(setTimeout(playShapesLoop, RESET_MS));
      }, SHAPE_TOTAL + GAP_MS));
    }
    ...
    const observer = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        started = true;
        /* 카드 1회 등장 → 고정 (이후 is-in 제거 안 함) */
        cards.forEach(c => c.classList.add('is-in'));
        /* 카드 등장과 동시에 도형 시퀀스 루프 시작 */
        playShapesLoop();
        observer.unobserve(entry.target);
      });
    }, { threshold: 0.2 });
